#ifndef CLICK_MINIONREWRITER_HH
#define CLICK_MINIONREWRITER_HH
...
MinionRewriter(INPUTSPEC1, ..., INPUTSPECn [, KEYWORDS])
...
In addition to IPRewriter's functionality, the MinionRewriter element can add or
...
dropped if an MinionRewriter's mapping table is full of guaranteed flows.
...
Returns a human-readable description of the MinionRewriter's current mapping
table.
...
class MinionRewriter : public TCPRewriter { public:
    ...
    MinionRewriter() CLICK_COLD;
    ~MinionRewriter() CLICK_COLD;

    const char *class_name() const		{ return "MinionRewriter"; }
...
inline void
MinionRewriter::destroy_flow(IPRewriterFlow *flow)
{
...
inline tcp_seq_t
MinionRewriter::TCPFlow::new_seq(bool direction, tcp_seq_t seqno) const
{
...
inline tcp_seq_t
MinionRewriter::TCPFlow::new_ack(bool direction, tcp_seq_t ackno) const
{
    delta_transition *dt = _dt;
    while (dt && dt->has_trigger(!direction)
